return {

-- Footer
["Settings"] = "Configurações",
["Launch"] = "Abrir",
["Details"] = "Detalhes",
["Category"] = "Categoria",
["View"] = "Ver",
["Close"] = "Fechar",
["Select"] = "Select",
["About"] = "Sobre",

-- General settings
["Language_colon"] = "Língua: ",
["Homebrews_Category_colon"] = "Categoria Homebrews: ",
["System_Apps_colon"] = "Aplicações de Sistema: ",
["Recently_Played_colon"] = "Tocado recentemente: ",
["Startup_scan_colon"] = "Verificação de inicialização: ",
["On"] = "Ligado",
["Off"] = "Desligado",

-- Appearance
["Custom_Background_colon"] = "Fundo Personalizado: ",
["Reflection_Effect_colon"] = "Efeito de Reflexão: ",
["Smooth_Scrolling_colon"] = "Rolagem suave: ",
["Theme_Color_colon"] = "Tema de Cor: ",
["Red"] = "Vermelho",
["Yellow"] = "Amarelo",
["Green"] = "Verde",
["Grey"] = "Cinza",
["Dark_Grey"] = "Cinza escuro",
["Black"] = "Preto",
["Purple"] = "Roxo",
["Dark_Purple"] = "Roxo escuro",
["Orange"] = "Laranja",
["Blue"] = "Azul",
["Views_2D_colon"] = "Ver 2D: ",
["Change_Views_colon"] = "Alterar ver: ",

-- Audio
["Audio"] = "Áudio",
["Sounds_colon"] = "Sons: ",
["Music_colon"] = "Música: ",
["Shuffle_music_colon"] = "Música aleatória: ",
["Skip_track"] = "Pular faixa",

-- Categories
["Edit_category_visibility"] = "Editar visibilidade das categorias",
["Startup_Category_colon"] = "Categoria Padrão: ",
["Last_played_game"] = "Último jogo jogado",
["Favorites"] = "Favoritos",
["Recently_Played"] = "Tocado recentemente",
["PS_Vita"] = "PS Vita",
["Homebrews"] = "Homebrews",
["PSP"] = "PSP",
["PlayStation"] = "PlayStation",
["Nintendo_64"] = "Nintendo 64",
["Super_Nintendo"] = "Super Nintendo",
["Nintendo_Entertainment_System"] = "Nintendo Entertainment System",
["Nintendo_DS"] = "Nintendo DS",
["Game_Boy_Advance"] = "Game Boy Advance",
["Game_Boy_Color"] = "Game Boy Color",
["Game_Boy"] = "Game Boy",
["Sega_Dreamcast"] = "Sega Dreamcast",
["Sega_Mega_Drive"] = "Sega Mega Drive",
["Sega_Master_System"] = "Sega Master System",
["Sega_Game_Gear"] = "Sega Game Gear",
["MAME_2000"] = "MAME 2000",
["Amiga"] = "Amiga",
["TurboGrafx_16"] = "TurboGrafx-16",
["TurboGrafx_CD"] = "TurboGrafx-CD",
["PC_Engine"] = "PC Engine",
["PC_Engine_CD"] = "PC Engine CD",
["Neo_Geo_Pocket_Color"] = "Neo Geo Pocket Color",
["Playstation_Mobile"] = "Playstation Mobile",
["Sega_CD"] = "Sega CD",
["Sega_32X"] = "Sega 32X",
["Commodore_64"] = "Commodore 64",
["WonderSwan_Color"] = "WonderSwan Color",
["WonderSwan"] = "WonderSwan",
["MSX2"] = "MSX2",
["MSX"] = "MSX",
["ZX_Spectrum"] = "ZX Spectrum",
["Atari_7800"] = "Atari 7800",
["Atari_5200"] = "Atari 5200",
["Atari_2600"] = "Atari 2600",
["Atari_Lynx"] = "Atari Lynx",
["ColecoVision"] = "ColecoVision",
["Vectrex"] = "Vectrex",
["FBA_2012"] = "FBA 2012",
["MAME_2003Plus"] = "MAME 2003 Plus",
["Neo_Geo"] = "Neo Geo",
["ScummVM"] = "ScummVM",
["PICO8"] = "PICO-8",
["System_Apps"] = "Aplicações de Sistema",
["Atari_ST"] = "Atari ST",
["MS_DOS"] = "MS-DOS",
["EasyRPG"] = "EasyRPG",

-- Download
["Download_colon"] = "Download:",
["Covers"] = "Capas",
["Backgrounds"] = "Fundos",
["Download_using_CRC_colon"] = "Baixe usando CRC:",
["Extract_PS_Vita_backgrounds"] = "Extrair planos de fundo do PS Vita",
["Extract_PSP_backgrounds"] = "Extrair planos de fundo do PSP:",
["Extract_PICO8_backgrounds"] = "Extrair planos de fundo do PICO-8",

["All"] = "Todos",
["Internet_Connection_Required"] = "Conexão com a Internet necessária",
["Cover"] = "Capa",
["Background"] = "Plano de fundo",
["Found"] = "Encontrada",
["found_exclamation"] = "encontrada!",
["Cover_not_found"] = "Capa não Encontrada",
["Background_not_found"] = "Plano de fundo não Encontrado",
["of"] = " de ",

["Downloading_covers"] = "Baixando capas",
["Downloading_backgrounds"] = "Baixando planos de fundo",

["No_missing_artwork"] = "Nenhuma obra de arte em falta",

-- Info Screen
["App_ID_colon"] = "ID do aplicativo: ",
["Size_colon"] = "Tamanho: ",
["Version_colon"] = "Versão: ",
["Download_Cover"] = "Baixar capa",
["Download_Background"] = "Baixar plano de fundo",
["Override_Category_colon"] = "Categoria de substituição: ",
["Press_X_to_apply_Category"] = "Pressione X para aplicar a categoria",
["Press_O_to_apply_Category"] = "Pressione O para aplicar a categoria",
["Default"] = "Padrões",
["Favorite"] = "Favorito",
["Rename"] = "Renomear",
["PS_Vita_Game"] = "Jogo PS Vita",
["Homebrew"] = "Homebrew",
["PSP_Game"] = "Jogo PSP",
["PS1_Game"] = "Jogo PlayStation",
["N64_Game"] = "Jogo Nintendo 64",
["SNES_Game"] = "Jogo Super Nintendo",
["NES_Game"] = "Jogo Nintendo Entertainment System",
["NDS_Game"] = "Jogo Nintendo DS",
["GBA_Game"] = "Jogo Game Boy Advance",
["GBC_Game"] = "Jogo Game Boy Color",
["GB_Game"] = "Jogo Game Boy",
["DC_Game"] = "Jogo Sega Dreamcast",
["MD_Game"] = "Jogo Sega Mega Drive",
["SMS_Game"] = "Jogo Sega Master System",
["GG_Game"] = "Jogo Sega Game Gear",
["MAME_2000_Game"] = "Jogo MAME 2000",
["Amiga_Game"] = "Jogo Amiga",
["TurboGrafx_16_Game"] = "Jogo TurboGrafx-16",
["TurboGrafx_CD_Game"] = "Jogo TurboGrafx-CD",
["PC_Engine_Game"] = "Jogo PC Engine",
["PC_Engine_CD_Game"] = "Jogo PC Engine CD",
["Neo_Geo_Pocket_Color_Game"] = "Jogo Neo Geo Pocket Color",
["Playstation_Mobile_Game"] = "Jogo Playstation Mobile",
["SCD_Game"] = "Jogo Sega CD",
["S32X_Game"] = "Jogo Sega 32X",
["C64_Game"] = "Jogo Commodore 64",
["WSWANCOL_Game"] = "Jogo WonderSwan Color",
["WSWAN_Game"] = "Jogo WonderSwan",
["MSX2_Game"] = "Jogo MSX2",
["MSX_Game"] = "Jogo MSX",
["ZXS_Game"] = "Jogo ZX Spectrum",
["A7800_Game"] = "Jogo Atari 7800",
["A5200_Game"] = "Jogo Atari 5200",
["A600_Game"] = "Jogo Atari 600",
["LYNX_Game"] = "Jogo Atari Lynx",
["COLECO_Game"] = "Jogo ColecoVision",
["VECTREX_Game"] = "Jogo Vectrex",
["FBA2012_Game"] = "Jogo FBA 2012",
["MAME2003_Game"] = "Jogo MAME 2003 Plus",
["Neo_Geo_Game"] = "Jogo Neo Geo",
["ScummVM_Game"] = "Jogo ScummVM",
["PICO8_Game"] = "Jogo PICO-8",
["System_App"] = "Aplicação de Sistema",
["Atari_ST_Game"] = "Jogo Atari ST",
["MS_DOS_Game"] = "Jogo MS-DOS",
["EasyRPG_Game"] = "Jogo EasyRPG",

-- Missing launcher message
["Please_install_RetroFlow_Adrenaline_Launcher"] = "Instale o RetroFlow Adrenaline Launcher.",
["The_VPK_is_saved_here"] = "O arquivo é salvo aqui",

-- Search
["Search"] = "Procurar",
["Search_Results"] = "Procurar Resultados",
["Search_No_Results"] = "Pressione selecionar para pesquisar novamente",

-- Random game
["Random_Game"] = "Juego aleatorio",

-- Settings Menu
["Categories"] = "Categorias",
["Sounds"] = "Sons",
["Artwork"] = "Obra de arte",
["Scan_Settings"] = "Configurações de digitalização",
["Adrenaline_roms"] = "Jogos de Adrenaline: ",
["All_Category"] = "Todas as categorias: ",
["Back_Chevron"] = "<  Voltar",
["Theme"] = "Tema",
["Game_backgounds_colon"] = "Fundos do jogo: ",

-- Game directories
["Home"] = "Pagina inicial",
["Directory_not_found"] = "Diretório não encontrado",
["Edit_game_directories"] = "Editar diretórios de jogos...",
["Game_directories"] = "Diretórios de jogos",
["Use_this_directory"] = "Use este diretório",
["Rescan"] = "Recarregar",
["Back"] = "Voltar",

-- Scan progress
["Scanning_titles"] = "Escaneando títulos...",
["Scanning_games_ellipsis"] = "Escaneando jogos...",
["Scan_complete"] = "Escaneamento concluído",
["Reloading_ellipsis"] = "Recarregando...",

-- Guides
["Help_and_Guides"] = "Ajuda",

["guide_1_heading"] = "Como adicionar jogos",
["guide_1_content"] = "Diretórios de jogos: \nColoque seus jogos em 'ux0:/data/RetroFlow/ROMS/', ou para usar seus próprios diretórios de arquivos, vá para 'Configurações de digitalização' e depois 'Editar diretórios de jogos'. \n\nDepois de adicionar seus jogos, selecione 'Recarregar' para adicioná-los ao RetroFlow. \n\nNomes de arquivos: \nÉ importante que seus jogos sejam nomeados usando a convenção de nomenclatura de arquivo 'no-intro', por exemplo, 'Sonic (USA)', caso contrário, as imagens não serão baixadas.",

["guide_2_heading"] = "Por que recebi um aviso do Adrenaline?",
["guide_2_content"] = "O RetroFlow não reconheceu o conjunto de módulos do Adrenaline instalado. Os jogos de PSP e PS1 continuarão a ser apresentados, mas poderão não iniciar corretamente.\n\nSe os jogos não iniciarem, atualize o RetroFlow quando estiver disponível uma versão mais recente.",
	
["guide_3_heading"] = "Personalize capas e fundos de jogos",
["guide_3_content"] = "Capas: \nCapas personalizadas podem ser salvas nas pastas do jogo aqui: 'ux0:/data/RetroFlow/COVERS/'. \n\nBackgounds: \nOs fundos de jogo personalizados podem ser salvos nas pastas do jogo aqui: 'ux0:/data/RetroFlow/BACKGROUNDS/'. \n\nNomes de arquivo:\nO nome do arquivo deve corresponder ao ID do aplicativo ou as imagens do nome do aplicativo devem estar no formato .png.",

["guide_4_heading"] = "Personalizar papel de parede e música",
["guide_4_content"] = "Papel de parede: \nVocê pode adicionar quantos papéis de parede quiser salvando-os aqui: 'ux0:/data/RetroFlow/WALLPAPER/'. \nAs imagens devem estar no formato .jpg ou .png e o tamanho deve ser 960px x 544px. \n\nMúsica: \nMúsicas podem ser adicionadas a 'ux0:/data/RetroFlow/MUSIC/'. \nA música deve estar no formato .ogg.",

["guide_5_heading"] = "Combinações de botões",
["guide_5_content"] = "Tecla de seta para cima: pula para a categoria de favoritos \n\nTecla de seta para baixo + quadrado: volta uma categoria \n\nTecla de seta para baixo + gatilhos L/R: pula jogos em ordem alfabética",

["guide_6_heading"] = "Sobre o aplicativo",
["guide_6_content"] = "RetroFlow by jimbob4000 é uma versão modificada do aplicativo HexFlow. \n\nO aplicativo HexFlow original é da VitaHex. Apoie os projetos da VitaHex em patreon.com/vitahex \n\nMais informações:\nPara mais informações e créditos completos, visite: https://github.com/jimbob4000/RetroFlow-Launcher",

-- Other Settings
["Other_Settings"] = "Outras configurações",
["Swap_X_and_O_buttons_colon"] = "Troque os botões X e O: ",
["Adrenaline_PS_button_colon"] = "Adrenaline botão PS: ",
["Menu"] = "Cardápio",
["LiveArea"] = "LiveArea",
["Standard"] = "Padrões",
["Show_missing_covers_colon"] = "Mostrar capas ausentes:",
["Time_colon"] = "Tempo:",
["Time_12hr"] = "Relógio de 12 horas",
["Time_24hr"] = "Relógio de 24 horas",
["Show_RetroArch_cores_colon"] = "Mostrar núcleos do RetroArch:",

-- Game options
["Options"] = "Opções",
["Adrenaline_options"] = "Opções de Adrenaline",
["Driver_colon"] = "Driver: ",
["Execute_colon"] = "Executar: ",
["Plugins_colon"] = "Plug-ins: ",
["CPU_speed_colon"] = "Velocidade da CPU: ",
["High_memory_colon"] = "Alta memória: ",
["NoDRM_engine_colon"] = "Motor de NoDRM: ",
["Suspend_threads_colon"] = "Suspender threads: ",
["Enable"] = "Habilitar",
["Disable"] = "Desativar",
["Yes"] = "Sim",
["No"] = "Não",

["Save"] = "Salvar",
["Add_to_favorites"] = "Adicionar aos favoritos",
["Remove_from_favorites"] = "Remover dos favoritos",
["Show_hidden_games_colon"] = "Mostrar jogos escondidos:",
["Hide_game"] = "Esconder jogo",
["Unhide_game"] = "Reexibir o jogo",
["Remove_from_recently_played"] = "Remover dos tocado recentemente",
["Configure_game_in_DSVita"] = "Configurar jogo no DSVita",

-- Collections
["Collections"] = "Coleções",
["Collections_colon"] = "Coleções:",
["Add_to_collection"] = "Adicionar a coleção",
["New_collection"] = "Nova coleção",
["New_collection_name"] = "Nome da nova coleção",
["Create"] = "Criar",
["Remove_from_collection"] = "Remover da coleção",
["No_collections"] = "Nenhuma coleção",
["Delete"] = "Excluir",
["Edit_collections"] = "Editar coleções",
["Show_collections_colon"] = "Mostrar coleções:",
["Customise_game_order"] = "Personalizar ordem dos jogos",
["Edit_collection_visibility"] = "Editar visibilidade das coleções",
["Show_all"] = "Mostrar todas",
["Hide_all"] = "Ocultar todas",

-- Adrenaline install assets
["RETROLNCR_Install"] = "Instalando RetroFlow Adrenaline Launcher vpk...",
["RETROLNCR_Installed"] = "RetroFlow Adrenaline Launcher foi instalado.",
["ABB_Install"] = "Instalando o plugin AdrBubbleBooter...",
["ABB_Installed"] = "O plugin do AdrBubbleBooter foi instalado...",
["ABB_Restart"] = "Precisamos reiniciar seu PS Vita.",
["Restart_Now"] = "Reiniciar agora",
["Restart_Later"] = "Reiniciar mais tarde",
["Install_Official_Adrenaline"] = "Instale o Adrenaline v7 do TheOfficialFloW para ativar jogos PSP e PS1.",
["Adrenaline_Unsupported"] = "O seu conjunto de módulos do Adrenaline não foi reconhecido. Os jogos de PSP e PS1 podem não iniciar.",
	
-- Launch messages
["Emulator_not_installed_Adrenaline"] = "Precisas de instalar o Adrenaline para jogar este jogo.",
["Emulator_not_installed_ScummVM"] = "Precisas de instalar o ScummVM para jogar este jogo.",
["Emulator_not_installed_Pico8"] = "Precisas de instalar o FAKE-08 para jogar este jogo.",
["Emulator_not_installed_Retroarch"] = "Precisas de instalar o RetroArch para jogar este jogo.",
["Emulator_not_installed_DaedalusX64"] = "Precisas de instalar o DaedalusX64 para jogar este jogo.",
["Emulator_not_installed_Flycast"] = "Precisas de instalar o Flycast para jogar este jogo.",
["Emulator_not_installed_DSVita"] = "Precisas de instalar o DSVita para jogar este jogo.",
["Emulator_not_installed_EasyRPG"] = "Precisas de instalar o EasyRPG para jogar este jogo.",
["Emulator_not_installed_Emu4Vita"] = "Precisas de instalar o Emu4Vita++ para jogar este jogo.",
["Game_not_installed_rescan"] = "Este jogo não está instalado, por favor, verifica os teus jogos de novo.",
["Insert_cartridge_try_again"] = "Insira o cartucho do jogo e tente novamente.",

-- Cartridges
["Scan_Vita_cartridges_colon"] = "Analisar cartuchos Vita:",

-- Emulator core strings
["Show_game_core_menu_colon"] = "Mostrar menu de núcleos dos jogos:",
["Global_core_settings"] = "Definições globais de núcleos",
["Platform_colon"] = "Plataforma:",
["Emulator_core"] = "Núcleo do emulador",
["Core_colon"] = "Núcleo:",
["Global_core_colon"] = "Núcleo global:",
["Reset_platform_core"] = "Repor núcleo da plataforma",
["Reset_all_platform_cores"] = "Repor todos os núcleos das plataformas",
["Reset_all_game_overrides"] = "Repor todas as substituições de jogos",
["Star_Recommended_core"] = "★ Núcleo recomendado",
["Use_Global"] = "Usar global",
["Core_source_retroarch"] = " (RetroArch)",
["Core_source_emu4vita"] = " (Emu4Vita++)",

-- Recovery Footer
["Confirm"] = "Confirmar",
["Cancel"] = "Cancelar",

-- Recovery Main screen
["Recovery_Tools"] = "Ferramentas de recuperação",
["Recovery_Tools_Info"] = "Utilize estas ferramentas se o RetroFlow deixar de iniciar corretamente.",
["Exit"] = "Sair",

-- Recovery Delete Cache
["Delete_Cache"] = "Eliminar cache",
["Info_Delete_Cache"] = "Elimina títulos em cache, bases de dados e ficheiros de cache gerados. Serão recriados automaticamente.",
["Confirm_Delete_Cache"] = "Todos os ficheiros de cache serão eliminados.",
["Success_Delete_Cache"] = "Cache eliminada com sucesso.",
["Failed_Delete_Cache"] = "Não foi possível eliminar a cache.",

-- Recovery Reset Config
["Reset_Config"] = "Repor configuração",
["Info_Config"] = "Elimina as definições do RetroFlow e repõe as definições padrão no próximo arranque.",
["Confirm_Config"] = "As definições do RetroFlow serão repostas.",
["Success_Config"] = "Configuração reposta com sucesso.",
["Failed_Config"] = "Não foi possível repor a configuração.",

-- Recovery Full Reset
["Full_Reset"] = "Reposição completa",
["Info_Full_Reset"] = "Repõe o RetroFlow para o estado padrão.",
["Confirm_Full_Reset"] = "Os seus dados atuais serão guardados como cópia de segurança. As suas ROMs e imagens não serão eliminadas.",
["Success_Full_Reset"] = "Os seus dados anteriores do RetroFlow foram guardados aqui:",
["Failed_Full_Reset"] = "Não foi possível criar a pasta de cópia de segurança.",
["No_Data_Folder"] = "A pasta de dados do RetroFlow não foi encontrada.",

}